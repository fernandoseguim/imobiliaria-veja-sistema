...
varCodIndicacao = request.QueryString("varCodIndicacao")
...
if varIndicacaoTipo <> "qualquer um" and varIndicacaoTipo <> "não informado" and varIndicacaoTipo <> "tqualquer" and varIndicacaoTipo <> "" then
...
if  varIndicacaoNegociacao <> "qualquer um" and varIndicacaoNegociacao <> "" then
...
if varIndicacaoValor <> "0" and varIndicacaoValor <> "" and varIndicacaoValor <> "0,00" then
...
'stringStandby = " and standby like '"&"suspenso"&"' and standby like '"&"comprador OK"&"'"
 ...
 stringStandby = " and (standby like 'comprador OK') and origem_franquia like '"&session("vOrigem_Franquia")&"' order by cod_compradores DESC "
...
function newWindow23232(abrejanela23232) {
   openWindow23232 = window.open(abrejanela23232,'openWin23232','width=700,height=500,resizable=yes,scrollbars=yes')
   openWindow23232.focus( )
   }
...
<table width="708" border="0" cellspacing="0" cellpadding="0">
          ...
          <td><table width="692" height="153" border="0" cellpadding="0" cellspacing="0">
              ...
              <tr> 
                      <td bgcolor="FE9225"><div align="center"><font face="Verdana, arial" size="1" color="#FFFFFF"><%=RS("descricao")%><br>
                          <br>
                          <a href="javascript:newWindow23232('form_enviar_email023.asp?varCodCompradores=<%=varCodComprador%>&varCod_imovel=<%=varCod_imovel%>')"><img src="bt_email22.jpg" width="25" height="18" border="0"></a><strong> 
                          Email : <%=rs("email")%></strong> ||<strong>Atendimento 
                          : <%=rs("atendimento")%></strong> || <strong>Comprador 
                          : <%=rs("nome")%></strong> || <strong>Data de inclusão 
                          : <%=rs("data")%></strong> <br>
                          <br>
                          <%if session("permissao") <> "4"  and  session("permissao") <> "5" and session("permissao") <> "6" then %>
                          <% if  UCase(rs("atendimento")) <> UCase(Session("Admin_ID")) and session("permissao") <> "6" then%>
                          <% else %>
                          <a href="javascript:newWindow33('visualizar_compradores33.asp?varCodCompradores=<%=varCodComprador%>')" style="color:#FFFFFF;text-decoration:none;"><strong>Veja 
                          a ficha completa na refer&ecirc;ncia <%=RS("cod_compradores")%> 
                          </strong></a></font></div>
                        <% end if %>
                        <%else%>
                        <font face="Verdana, arial" size="1" color="#FFFFFF"><a href="javascript:newWindow33('visualizar_compradores33.asp?varCodCompradores=<%=varCodComprador%>')" style="color:#FFFFFF;text-decoration:none;"><strong>Veja 
                        a ficha completa na refer&ecirc;ncia <%=RS("cod_compradores")%> </strong></a></font><%end if%> 
                      </td>
              </tr>
            </table></td>
  ...
  <tr>
    <td><table width="518" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td><div align="center"><font face="Verdana, arial" size="1"> 
            ...
            <!-- se a página atual for maior que "1" então o link anteriro é colocado na 
			  na tela .-->
             <a href="?page=<%=intPage - 1%>&varIndicacaoCidade=<%=varIndicacaoCidade%>&varIndicacaoBairro=<%=varIndicacaoBairro%>&varIndicacaoTipo=<%=varIndicacaoTipo%>&varIndicacaoNegociacao=<%=varIndicacaoNegociacao%>&varIndicacaoQuartos=<%=varIndicacaoQuartos%>&varIndicacaoVagas=<%=varIndicacaoVagas%>&varIndicacaoValor=<%=varIndicacaoValor%>&varCodCompradores=<%=varCodComprador%>&varCod_imovel=<%=varCod_imovel%>"> 
        ...
        <td> 
          ...
          <a href="?page=<%=intPage + 1%>&varIndicacaoCidade=<%=varIndicacaoCidade%>&varIndicacaoBairro=<%=varIndicacaoBairro%>&varIndicacaoTipo=<%=varIndicacaoTipo%>&varIndicacaoNegociacao=<%=varIndicacaoNegociacao%>&varIndicacaoQuartos=<%=varIndicacaoQuartos%>&varIndicacaoVagas=<%=varIndicacaoVagas%>&varIndicacaoValor=<%=varIndicacaoValor%>&varCodCompradores=<%=varCodComprador%>&varCod_imovel=<%=varCod_imovel%>"><font color="#000000" size="1" face="Verdana, Arial, Helvetica, sans-serif"><strong>Próximo</strong></font></a></td><% end if%>
        </tr>
      </table></td>
  </tr>
</table>
