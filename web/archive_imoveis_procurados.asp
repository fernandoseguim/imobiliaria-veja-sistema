...
Set rs = Server.CreateObject("ADODB.RecordSet")
  ...
  Conexao.execute"Insert into visualiza_paginas(visualiza_pagina_nome, visualiza_pagina_pagina, visualiza_pagina_data, origem_franquia) values( '"& session("nome_id") &"','"& "Imóveis procurados" &"','"& now() &"','"& session("vOrigem_Franquia") &"')" 
...
%><br>
<center><font size="1" face="Verdana, Arial, Helvetica, sans-serif" color="#000000"><%rs.movelast%><strong><%=rs("data")%></strong> até  <%rs.movefirst%><strong><%=rs("data")%></strong></strong><br><br> foram <strong><%=rs.RecordCount%></strong> acessos.</font></center>
...
<% end if%>
		  ...
		  <td width="115" height="30" bgcolor="<%=color1%>" style="border:1px solid #FFFFFF;">
<div align="center"><font color="#FFFFFF" size="1" face="Verdana, Arial, Helvetica, sans-serif">
          <%if session("permissao") <> "6" then response.write "não informado" else response.write rs("email") end if%>&nbsp;
          </font></div></td>
...
<div align="center"I><font color="<%=escuro%>">Imóvel não encontrado</font></div>
</font> 
<%
End If%>
<%else%>
