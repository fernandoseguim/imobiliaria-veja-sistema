  ...
  <table width="900" border="0" cellspacing="0" cellpadding="0">
    <tr> 
      ...
      <td width="26" height="18" bgcolor="<%=claro%>"> 
        <% if  (session("permissao") = "3" or  session("permissao") = "5" or session("permissao") = "6") and (UCase(session("vOrigem_Franquia")) = UCase(rs("origem_franquia")))  then %>
...
'se no cliente ou no servidor.
	...
	strSQL444P = "SELECT compradores.cod_compradores,compradores.telefone,compradores.telefone02,compradores.telefone03 FROM compradores where (telefone like '"&rs("telefone")&"' or telefone02 like '"&rs("telefone")&"' or telefone03 like '"&rs("telefone")&"') and origem_franquia like '"&session("vOrigem_Franquia")&"'"
	...
	vPerguntaPermuta = "sim"
	   ...
	   <table width="580" border="0" cellspacing="0" cellpadding="0">
			   ...
			   <tr> 
                  ...
                  <td bgcolor="<%=claro%>" width="290" style="border:1px solid #FFFFFF;">&nbsp;<font color="#FFFFFF" size="2" face="Verdana, Arial, Helvetica, sans-serif"><strong><%=rs444Permuta("origem_franquia")%></strong></font></td>
              </tr>
...
<div align="center"><font color="#FFFFFF" size="2" face="Verdana, Arial, Helvetica, sans-serif"><strong>Que 
      ...
      <td width="25" height="18" bgcolor="<%=color1%>" style="border:1px solid #FFFFFF;">
	   <%'if (session("permissao") = "1" and rs("captacao") <> session("nome_id")) or rs("origem_franquia") <> session("vOrigem_Franquia") then  %>
	 <%if (session("permissao")) = "" or (session("permissao") = "3") then  %>
...
<%end if%>			   
	  ...
	  <%if (session("permissao") = "1" and rs("captacao") <> session("nome_id")) or session("permissao") = "3" then  %>
	   ...
	   <%
		...
		<table width="570" border="0" cellspacing="0" cellpadding="0">
		   ...
		   <tr> 
                  <td bgcolor="<%=claro%>" style="border:1px solid #FFFFFF;" width="290"> 
                    <div align="center"><font color="#FFFFFF" size="2" face="Verdana, Arial, Helvetica, sans-serif"><strong>Franquia</strong></font></div></td>
                  <td bgcolor="<%=claro%>" width="290" style="border:1px solid #FFFFFF;">&nbsp;<font color="#FFFFFF" size="2" face="Verdana, Arial, Helvetica, sans-serif"><strong><%=rs("origem_franquia")%></strong></font></td>
              </tr>
...
<%
 ...
 SQL444Compradores202 = "SELECT compradores.cod_compradores,compradores.telefone,compradores.telefone02,compradores.telefone03,compradores.cod_compradores,compradores.tipo,compradores.origem_franquia FROM compradores where (telefone like '"&rs("telefone")&"' or telefone02 like '"&rs("telefone")&"' or telefone03 like '"&rs("telefone")&"' )  and origem_franquia like '"&session("vOrigem_Franquia")&"'"
	...
	if  (not rs202Compradores.eof)   then
		...
		%>
		<% if  (rs202Compradores("origem_franquia") <> session("vOrigem_Franquia"))  then %>
    ...
    <td bgcolor="#FFFFFF"> 
      <div align="center"><font face="Verdana, arial" size="1" color="#000000"> 
        ...
        <!-- se a página atual for maior que "1" então o link anteriro é colocado na 
			  na tela .-->
        <a href="?page=<%=intPage - 1%>&varCidade=<%=session("varCidade")%>&varCidade2=<%=session("varCidade2")%>&varBairro=<%=session("varBairro")%>&varBairro2=<%=session("varBairro2")%>&varNegociacao=<%=session("varNegociacao")%>&varTipo=<%=session("varTipo")%>&varQuartos=<%=session("varQuartos")%>&varVagas=<%=session("varVagas")%>&SearchFor=<%=session("SearchFor")%>&SearchWhere=<%=session("SearchWhere")%>&varValor=<%=session("varValor")%>&varValor1=<%=session("varValor1")%>&varValor2=<%=session("varValor2")%>&varFoto=<%=session("varFoto")%>&varDevedor=<%=session("varDevedor")%>&varSuites=<%=session("varSuites")%>&varPiscina=<%=session("varPiscina")%>&varPortaria=<%=session("varPortaria")%>&varQuintal=<%=session("varQuintal")%>&varQuadras=<%=session("varQuadras")%>&varEdicula=<%=session("varEdicula")%>&varCondominio=<%=session("varCondominio")%>&varAreaTotal=<%=session("varAreaTotal")%>&varAreaTotal1=<%=session("varAreaTotal1")%>&varAreaTotal2=<%=session("varAreaTotal2")%>&varOcupacao=<%=session("varOcupacao")%>&varCaptacao=<%=session("varCaptacao")%>&varStandbyImovel=<%=session("varStandbyImovel")%>" style="color:#000000"> 
...
<%
 ...
 '----------------
'response.write SQL 
'response.write session("vOrigem_Franquia")&"!!"&session("vOrigem_Franquia02")
%>
