...
rs444Responsavel.ActiveConnection = Conexao
	...
	dim rs444Responsavel77,strSQL444Responsavel77
   
    Set rs444Responsavel77 = Server.CreateObject("ADODB.RecordSet")
	strSQL444Responsavel77 = "SELECT senha.list_name,senha.admin_id,senha.admin_pass,senha.admin_email,senha.from_email,senha.url_dir,senha.url_home,senha.component,senha.smtp,senha.permissao,senha.id  FROM senha  ORDER BY id Desc" 
	
	
	rs444Responsavel77.CursorLocation = 3
'a propriedade CursorLocation do objeto recordset indica onde o cursor é criado
'se no cliente ou no servidor.

rs444Responsavel77.CursorType = 3
'indica o tipo de cursor utilizado, se somente de leitura ou se de leitura e gravação.

rs444Responsavel77.ActiveConnection = Conexao
	
	
	
	
	 rs444Responsavel77.Open strSQL444Responsavel77, Conexao
...
<div align="center"> <%
	...
	%>
          <%else%>
          <font color="#FFFFFF" size="1" face="Verdana, Arial, Helvetica, sans-serif">O Imóvel 
          foi incluido com sucesso.</font> 
          <% end if %>
		  </div>
		  
		  
<form name="doublecombo"  onSubmit="return isValidDigitNumber(this);" method="post" action="colaborador_incluir_imovel33.asp">
    ...
    <td> <table width="1000" cellpadding="0" cellspacing="0">
        ...
        <tr> 
          ...
          <td width="192" bgcolor="<%=medio%>" style="border:1px solid #FFFFFF;"><select name="txt_responsavel_cadastramento_vend" id="txt_responsavel_cadastramento_vend" class="inputBox" style="HEIGHT: 18px; WIDTH: 190px; background: <%=medio%>">
				...
				<option value="internet" >internet</option>
					  <option value="Internet">Internet</option>
                      <% if not rs444Responsavel77.eof then %>
                      <% While NOT rs444Responsavel77.EoF %>
                      <option value="<% = rs444Responsavel77("list_name") %>"> 
                      <% = rs444Responsavel77("list_name") %>
                      </option>
                      <% rs444Responsavel77.MoveNext %>
                      <% Wend %>
                      <%else%>
                      <option value="Internet">Internet</option>
                      <%end if%>
