...
if (doublecombo.txt_ocupacao_vend.value == "não informado" || doublecombo.txt_ocupacao_vend.value == ""  ) {
        alert("Você precisa indicar o se o imóvel está vago ou não!");
        doublecombo.txt_ocupacao_vend.focus();
		
        return false;
    }

var strValidNumber1_condominio7="1234567890,.";
for (nCount=0; nCount < doublecombo.txt_condominio_vend.value.length; nCount++) 
		{
strTempChar1_condominio7=doublecombo.txt_condominio_vend.value.substring(nCount,nCount+1);
if (strValidNumber1_condominio7.indexOf(strTempChar1_condominio7,0)==-1) 
{
alert("O item condomínio só pode conter números!");
...
for (nCount=0; nCount < doublecombo.txt_a_constr_vend.value.length; nCount++) 
		{
strTempChar9_ja=doublecombo.txt_a_constr_vend.value.substring(nCount,nCount+1);
...
doublecombo.txt_a_constr_vend.select();
return false;
}
}
...
if (doublecombo.txt_a_total_vend.value == "" || doublecombo.txt_a_total_vend.value == "0") {
        alert(" Você precisa informar se a metragem do imóvel!");
        doublecombo.txt_a_total_vend.focus();
		doublecombo.txt_a_total_vend.select();
        return false;
    }
...
if (doublecombo.txt_obs_imovel_vend.value == "") {
        alert(" Você precisa informar as observações do imóvel e forma de pagamento!");
...
if (doublecombo.txt_email_vend.value == "") {
        alert(" Você precisa informar o email do proprietário do imóvel!");
...
<% if rs("pergunta") <> "não" then %>
	
	if ( doublecombo.txt_pergunta.value == "não informado") {
        alert("Você precisa informar se o proprietário desse imóvel também é comprador");
        doublecombo.txt_pergunta.focus();
...
<% if session("permissao") <> "6" then %>
	...
	if ( doublecombo.txt_telefone_vend.value != <%=rs("telefone")%>) {
        alert("Você não pode trocar esse telefone!");
        doublecombo.txt_telefone_vend.focus();
...
<div align="center"><font color="#FFFFFF" size="1" face="Verdana, Arial, Helvetica, sans-serif">Que 
    ...
    <td width="200" height="20"> 
              <div align="center"><font color="<%=letra%>" size="4" face="Verdana, Arial, Helvetica, sans-serif"><strong>Im&oacute;vel</strong></font><br>
                ...
                de financiamento</a> <br><% if rs("imovel_em_negociacao") = "Vendido pela Veja" then %> <a href="javascript:newWindow77('form_venda_realizada01.asp?varCod_imovel=<%=varCod_imovel%>')" style="color:<%=letra%>">Dados da venda realizada</a><%else%><%end if%><br><a href="javascript:newWindow88('imprimir_imovel22.asp?varCod_imovel=<%=varCod_imovel%>')" style="color:<%=letra%>">Imprimir</a><br><br><% if session("permissao") = "6"  then %> <a href="javascript:newWindow808('visualizar_historico33.asp?varCod_imovel=<%=varCod_imovel%>')" style="color:<%=letra%>">Histórico de atualizações</a><%else%><%end if%>
				<br>
				<br>
				<a href="javascript:newWindow3121('form_duplicar_imovel01.asp?varCod_imovel=<%=varCod_imovel%>')" style="color:<%=letra%>">Duplicar</a>
...
<input name="txt_valor_vend" type="text" class="inputBox" id="txt_valor_vend" style="HEIGHT: 18px; WIDTH: 190px; background:<%=medio%>" value="<%=FormatNumber(rs("valor"),2)%>" size="12" maxlength="13"></td>
  ...
  <tr> 
            <td width="195" bgcolor="<%=escuro%>" ><font color="<%=letra%>" size="1" face="Verdana, Arial, Helvetica, sans-serif">Primeira 
        ...
        <tr> 
            <td width="195" bgcolor="<%=medio%>" style="border:1px solid #FFFFFF;"><select name="txt_presenca_primeira_vend" id="txt_presenca_primeira_vend" size="1" class="inputBox" style="HEIGHT: 18px; WIDTH: 190px; background:<%=medio%>">
  ...
  <tr> 
            <td width="195" bgcolor="<%=escuro%>" ><font color="<%=letra%>" size="1" face="Verdana, Arial, Helvetica, sans-serif">Quem 
        ...
        <tr> 
            <td width="195" bgcolor="<%=medio%>" style="border:1px solid #FFFFFF;"><select name="txt_quem_tirou_foto_vend" id="txt_quem_tirou_foto_vend" class="inputBox" style="HEIGHT: 18px; WIDTH: 190px; background: <%=medio%>">
	...
	<table width="1000" cellpadding="0" cellspacing="0">
        ...
        <tr> 
          ...
          <td width="192" bgcolor="<%=medio%>" style="border:1px solid #FFFFFF;"><select name="txt_responsavel_cadastramento_comprador" id="txt_responsavel_cadastramento_comprador" class="inputBox" style="HEIGHT: 18px; WIDTH: 190px; background: <%=medio%>">
					 ...
					 <option value="<% if rs444Comprador("responsavel_cadastramento") <> "não informado" and rs444Comprador("melhor_horario_visita") <> "" then response.write rs444Comprador("responsavel_cadastramento") else response.write rs("captacao") end if%>" selected><% if rs444Comprador("responsavel_cadastramento") <> "não informado" and rs444Comprador("responsavel_cadastramento") <> "" then response.write rs444Comprador("responsavel_cadastramento") else response.write rs("captacao") end if%></option>
          ...
          <td width="192" bgcolor="<%=medio%>" style="border:1px solid #FFFFFF;"><select name="txt_atendimento" id="txt_atendimento" class="inputBox" style="HEIGHT: 18px; WIDTH: 190px; background: <%=medio%>">
					 ...
					 <option value="<% if rs444Comprador("atendimento") <> "não informado" and rs444Comprador("atendimento") <> "" then response.write rs444Comprador("atendimento") else response.write "não informado" end if%>" selected><% if rs444Comprador("atendimento") <> "não informado" and rs444Comprador("atendimento") <> "" then response.write rs444Comprador("atendimento") else response.write "não informado" end if%></option>
  ...
  <tr> 
    <td><table width="1000" border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td width="10">&nbsp;</td>
          <td width="495" height="80" bgcolor="<%=medio%>" style="border:1px solid #FFFFFF;"><textarea name="txt_descricao_confi" COLS=20 ROWS=10 class="inputBox" id="txt_descricao_confi" style="HEIGHT: 78px; WIDTH: 493px; background:<%=medio%>" onKeyPress="return limitfield(this, 800)"><%=rs444Comprador("descricao_confi")%></textarea></td>
        </tr>
      </table></td>
  </tr>
